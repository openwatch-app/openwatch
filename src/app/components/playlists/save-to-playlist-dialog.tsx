...
export const SaveToPlaylist = ({ videoId, children }: SaveToPlaylistProps) => {
	...
	const togglePlaylist = async (playlistId: string, currentHasVideo: boolean) => {
		...
		try {
			...
			const errorMessage = error.response?.data?.error || t('playlists.dialog.error_update');
	...
	const handleCreatePlaylist = async () => {
		...
		try {
			...
			const errorMessage = error.response?.data?.error || t('playlists.dialog.error_create');
			alert(errorMessage);
		} finally {
			setCreating(false);
		}
		...
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent align="end" className="w-72 p-0 overflow-hidden bg-background border-border/50 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
				...
				<div className="flex items-center justify-between p-3 border-b border-border/50 bg-secondary/10">
					<h2 className="font-semibold text-sm">{t('playlists.dialog.save_to')}</h2>
				...
				{!showCreateForm ? (
					<div className="p-2 border-t border-border/50 bg-secondary/5">
						<Button variant="ghost" className="w-full justify-start gap-2 h-9 text-xs rounded-md hover:bg-secondary/80 transition-all" onClick={() => setShowCreateForm(true)}>
							...
							<span>{t('playlists.dialog.create_new')}</span>
						</Button>
					</div>
				) : (
						...
						<div className="space-y-2.5">
							<div>
								...
								<Input
									...
									placeholder={t('playlists.dialog.name_placeholder')}
							...
							<div>
								<label className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground/70 mb-1 block px-1">{t('playlists.dialog.visibility')}</label>
								<select
							...
							<div className="flex justify-end gap-1.5 pt-1">
								...
								<Button className="h-7 px-3 text-[11px] shadow-sm" disabled={!newPlaylistTitle.trim() || creating} onClick={handleCreatePlaylist}>
									{creating ? t('playlists.dialog.creating') : t('playlists.dialog.create')}
								</Button>
							</div>
						</div>
					</div>
